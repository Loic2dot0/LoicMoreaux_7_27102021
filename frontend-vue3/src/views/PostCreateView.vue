<template>
     <div v-if="session">
        ...
        <section>
            ...
            <form @submit="createPost" v-else>
                <span class="success" v-if="message">{{ message }}</span>
                <span class="error" v-if="error.global">{{ error.global }}</span>
                <input type="file" accept="image/png, image/jpeg, image/jpg, image/webp, image/gif" ref="file" @change="upload()">
                ...
                <input type="text" name="title" maxlength="140"
                    ...
                    @keyup="validText(formData.title)"
                    :class="{valid: valid.title}"
                >
                <input type="submit" value="Publier" class="btnSubmit" :disabled=btnDisabled>
            </form>
        </section>
    </div>
</template>
