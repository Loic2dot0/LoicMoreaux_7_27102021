<template>
    <div v-if="session" class="div-global">
        <Profile :key="reload"></Profile>
        <section>
            <div>
                <h2>Modifier mon profil</h2>

                <form @submit="modifyProfile">
                    <span class="success" v-if="message">{{ message }}</span>
                    <input type="file" accept="image/png, image/jpeg, image/jpg, image/webp" ref="file" @change="upload()">
                    ...
                    <select name="service" v-model="formData.service">
                        <option :key="index" v-for="(service, index) in serviceList" >{{ service }}</option>
                    </select>
                    <input type="submit" value="Modifier mon compte" class="btnSubmit" :disabled=btnDisabled>
                </form>
            </div>

            <div class="div-delete">
                <button @click="modal=true" class="btn-delete">Supprimer mon compte</button>
            </div>
        </section>
        <ModalDelete v-if="modal" @close="modal=false"></ModalDelete>
    </div>
</template>
